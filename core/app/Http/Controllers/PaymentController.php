...
// Set the merchant ID and secret key (can be found in account settings on CoinPayments.net)
            $CP->setMerchantId($method->val2);
            $CP->setSecretKey($method->val1);

// Create a payment button with item name, currency, cost, custom variable, and the callback URL

            $ntrc = $data->trx;

            $form = $CP->createPayment('Purchase Coin', 'btc',  $bcoin, $ntrc, $callbackUrl, $successUrl, $cancelUrl);
        ...
        elseif($data->gateway_id ==8)
        {
            ...
            $varb = "bitcoincash:".$DepositData->bcid ."?amount=".$DepositData->bcam;
		...
		elseif($data->gateway_id ==9)
        {
                    ...
                    $tradingcycle = date('Y-m-d', strtotime(config('constants.commission_cycle.trading_cycle')));
                    Transaction::create([
                    ...
                    // Insert in wallet 
                    $directwallet_per = config('constants.wallet.directmode');
                    ...
                    $direct_amount = ($data['amount']*$directwallet_per)/100;
                    $cash_amount = ($data['amount']*$cashwallet_per)/100;
                    $trader_amount = ($data['amount']*$traderwallet_per)/100;
                    $custom_amount = ($data['amount']*$customwallet_per)/100;
                    $wallet_check = Wallet::where('user_id', $data['user_id'])->first();
                    if(empty($wallet_check)){
                        ...
                        //DB::enableQueryLog();
                        Wallet::where('user_id',$data['user_id'])
                               ->update([
                                  'direct_amount' => $direct_amount+$wallet_check['direct_amount'],
                    ...
                    curl_setopt_array($curl, array(
                      CURLOPT_URL => "http://www.tradepromarket.com/wallet_balance_api.php?action=set&amount=".$data['amount']."&email=".$email,
...
// Set TCP timeout to 30 seconds
        ...
        if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0)
        {
            ...
            if($receiver_email==$gatewayData->val1 && $mc_currency=="USD" && $mc_gross ==$amount && $DepositData->status=='0')
            {
				...
				$direct_amount = ($bdt_amount*$directwallet_per)/100;
				$cash_amount = ($bdt_amount*$cashwallet_per)/100;
				$trader_amount = ($bdt_amount*$traderwallet_per)/100;
                $custom_amount = ($bdt_amount*$customwallet_per)/100;
				$wallet_check = Wallet::where('user_id', $DepositData['user_id'])->first();
				if(empty($wallet_check)){
					...
					Wallet::where('user_id',$DepositData['user_id'])
						   ->update([
							  'direct_amount' => $direct_amount+$wallet_check['direct_amount'],
								'cash_amount' =>$cash_amount+$wallet_check['cash_amount'],
								'trader_amount' => $trader_amount+$wallet_check['trader_amount'],
                ...
                //updateReferralincomeOndeposit($DepositData['user_id'],$bdt_amount);
    ...
    public function ipnperfect()
    {
        ...
        if($hash==$hash2){
            ...
            if($_POST['PAYEE_ACCOUNT']==$gatewayData->val1 && $unit=="USD" && $amo ==$amount && $DepositData->status=='0'){
				...
				$direct_amount = ($DepositData['amount']*$directwallet_per)/100;
				$cash_amount = ($DepositData['amount']*$cashwallet_per)/100;
				$trader_amount = ($DepositData['amount']*$traderwallet_per)/100;
                $custom_amount = ($DepositData['amount']*$customwallet_per)/100;
				$wallet_check = Wallet::where('user_id', $DepositData['user_id'])->first();
				if(empty($wallet_check)){
    ...
    public function ipnbtc(){
        ...
        if ($DepositData->status == 0) {
            ...
            if ($DepositData->bcam==$value_in_btc && $DepositData->bcid==$address && $secret=="ABIR" && $confirmations>2){
                ...
                //updateReferralincomeOndeposit($DepositData['user_id'],$DepositData['amount']);
                // Insert in wallet 
				$directwallet_per = config('constants.wallet.directmode');
				$cashwallet_per = config('constants.wallet.cashtmode');
				$traderwallet_per = config('constants.wallet.tradermode');
				...
				$cash_amount = ($DepositData['amount']*$cashwallet_per)/100;
				$trader_amount = ($DepositData['amount']*$traderwallet_per)/100;
                $custom_amount = ($DepositData*$customwallet_per)/100;
				$wallet_check = Wallet::where('user_id', $DepositData['user_id'])->first();
				if(empty($wallet_check)){
        ...
        try{
            ...
            try{
                ...
                if ($charge['status'] == 'succeeded') {
					...
					//updateReferralincomeOndeposit($data['user_id'],$data['amount']);
					// Insert in wallet 
					$directwallet_per = config('constants.wallet.directmode');
					$cashwallet_per = config('constants.wallet.cashtmode');
					$traderwallet_per = config('constants.wallet.tradermode');
    ...
    public function coingatePayment()
    {
        ...
        $post_params = array(
            ...
            'currency'          => 'BTC',
            'receive_currency'  => 'BTC',
            'callback_url'      => url('/api/ipncoin-gate'),
            'cancel_url'        => route('home'),
            'success_url'       => route('home'),
            'title'             => 'AF#'.$DepositData->trx,
            'description'       => 'Add Fund'
