...
class TicketController extends Controller
{
    public function ticketIndex()
    {
        ...
        $get_ticket = Ticket::where(['customer_id'=>Auth::user()->id,'status'=>2])->get();
          foreach($get_ticket as $get_ticket123){
            TicketComment::where(['ticket_id'=>$get_ticket123->ticket])->update(['status'=>1]);
        }
    ...
    public function ticketCloseadmin($id)
    {
        Ticket::where('id', $id)->update(['status'=>9]);
        return redirect()->back()->with('message', 'Ticket Closed Successful');
    }
